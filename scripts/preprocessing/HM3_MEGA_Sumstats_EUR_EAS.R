...
# dx run app-swiss-army-knife -iin=PreConfluenceAnalysis:Aim1_Heritability/JacobWilliams/Clean_Data_Scripts/r_with_plink.tar.gz -iin=PreConfluenceAnalysis:Aim1_Heritability/JacobWilliams/Clean_Data_Scripts/HM3_MEGA_Sumstats_EUR_EAS.R -iin=PreConfluenceAnalysis:Aim1_Heritability/JacobWilliams/Clean_Data_Scripts/HM3_MEGA_Sumstats_EUR_EAS.sh -icmd="bash HM3_MEGA_Sumstats_EUR_EAS.sh" -y --destination PreConfluenceAnalysis:Aim2_Polygenicity/HZ/Results/Clean_summary_data_update/ --instance-type mem1_ssd1_v2_x36
...
write.table(EAS_data_sub_quan, file = "EAS_Overall_HM3_MEGA_01.txt", row.names = F, col.names = T, quote = F)
...
write.table(EAS_data_sub_quan, file = "EAS_Overall_HM3_MEGA_05.txt", row.names = F, col.names = T, quote = F)
...
Eur_Overall_sub <- Eur_Overall %>% 
  ...
  filter(SNP_ID %in% hm3_mega$rsid)
...
write.table(Eur_Overall_sub_quan, file = "EUR_Overall_HM3_MEGA_01.txt", row.names = F, col.names = T, quote = F)
...
write.table(Eur_Overall_sub_quan, file = "EUR_Overall_HM3_MEGA_05.txt", row.names = F, col.names = T, quote = F)
...
Eur_ERpos_Overall_sub <- Eur_ERpos_Overall %>% 
  ...
  filter(Freq_effect_Onco >= 0.01 & Freq_effect_Onco <= 0.99) %>%
  filter(phase3_1kg_id %in% hm3_mega$rsid)
...
write.table(Eur_ERpos_Overall_sub_quan, file = "EUR_ERPos_HM3_MEGA_01.txt", row.names = F, col.names = T, quote = F)
...
write.table(Eur_ERpos_Overall_sub_quan, file = "EUR_ERPos_HM3_MEGA_05.txt", row.names = F, col.names = T, quote = F)
...
write.table(Eur_ERneg_Overall_sub_quan, file = "EUR_ERNeg_HM3_MEGA_01.txt", row.names = F, col.names = T, quote = F)
...
write.table(Eur_ERneg_Overall_sub_quan, file = "EUR_ERNeg_HM3_MEGA_05.txt", row.names = F, col.names = T, quote = F)
