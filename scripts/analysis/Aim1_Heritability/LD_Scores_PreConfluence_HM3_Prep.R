...
write.table(w_hm3_ldsc[,1,drop = FALSE],file = "/data/williamsjacr/1KG_ldsc_HM3/w_hm3_ldsc_printsnps.snplist",row.names = FALSE,quote=F,sep = '\t',col.names = FALSE)
...
for(i in 1:length(eth)){
  ...
  local_dir <- paste0("/data/williamsjacr/1KG_ldsc_HM3/",eth[i],"/")
  ...
  for(j in 1:22){
    ...
    system(paste0("/data/williamsjacr/software/plink2 --bfile ",kg.dir,"chr",j," --keep ",local_dir,"keeplist --ref-allele /data/williamsjacr/ldsc/ref_allele_hm3.txt --make-bed --out ",local_dir,"chr_clean",j))
